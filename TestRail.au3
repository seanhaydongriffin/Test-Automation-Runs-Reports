...
Func _TestRailGetResultsForRun($run_id, $created_after, $created_before)
	...
	Local $iPID = Run('curl.exe -k -H "Content-Type: application/json" -u ' & $testrail_username & ':' & $testrail_password & ' ' & $testrail_domain & '/index.php?/api/v2/get_results_for_run/' & $run_id & "&created_after=" & $created_after & "&created_before=" & $created_before, @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
    ProcessWaitClose($iPID)
    $testrail_json = StdoutRead($iPID)
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $testrail_json = ' & $testrail_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
...
;		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $testrail_json = ' & $testrail_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		...
		for $j = 0 to 99999
			...
			Local $id = Json_Get($decoded_json, '.plan.entries[0].runs[' & $j & '].id')
			ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $id = ' & $id & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
